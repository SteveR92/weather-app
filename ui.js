class UI {
    ...
    const desc = this.desc = weather.weather[0].main;
    const string = this.string.textContent = Math.floor(weather.main.temp) + '°C';
    ...
    if(weather.rain){
    const value = weather.rain;
    const rainValue = parseFloat(Object.values(value));
    ...
    const updateWeather1 = this.cardText1.innerText;
    this.cardText2.innerText = weather.list[4].weather[0].main;
    const updateWeather2 = this.cardText2.innerText;
    this.cardText3.innerText = weather.list[5].weather[0].main;
    const updateWeather3 = this.cardText3.innerText;

    //card temp text
    const updateTemp1 = this.cardTemp1.innerText = Math.floor(weather.list[3].main.temp) + '°C';
    const updateTemp2 = this.cardTemp2.innerText = Math.floor(weather.list[4].main.temp) + '°C';
    const updateTemp3 = this.cardTemp3.innerText = Math.floor(weather.list[5].main.temp) + '°C';

    const rainRate = this.rainRating;
    const minNum = document.querySelector('.mins');
    const hrsNum = document.querySelector('.hrs');
    const colon = document.querySelector('.colon1');
    const weatherIcon = this.icon;
    const white = "white";

    //card images

    const cardImg1 = this.forecastImage1.getAttribute('src').split('k').pop();
    const finalImg1 = white + cardImg1;

       
    const cardImg2 = this.forecastImage2.getAttribute('src').split('k').pop();
    const finalImg2 = white + cardImg2;

       
    const cardImg3 = this.forecastImage3.getAttribute('src').split('k').pop();
    const finalImg3 = white + cardImg3;

    const updateWeather = this.weatherInfo;

    //color
    const menuColor = this.menuBackground;
    const mainColor = this.updateColor;
    ...
    this.card1.addEventListener('click', function(){
        ...
        if(weather.list[4].rain){
            const value = weather.list[4].rain;
            const rainValue = parseFloat(Object.values(value));
    ...
    //update main info card with forecast
      this.card3.addEventListener('click', function(){
        ...
        if(weather.list[5].rain){
           const value = weather.list[5].rain;
           const rainValue = parseFloat(Object.values(value));
